...
SQL;
    ...
    public function query($query)
    {
        ...
        if ($query_type == \YoastSEO_Vendor\SQL_SELECT || $query_type == \YoastSEO_Vendor\SQL_SHOW) {
            $res = \YoastSEO_Vendor\pg_query($this->conn, $query);
            if ($this->isError($res)) {
                throw new \YoastSEO_Vendor\Ruckusing_Exception(\sprintf("Error executing 'query' with:\n%s\n\nReason: %s\n\n", $query, \YoastSEO_Vendor\pg_last_error($this->conn)), \YoastSEO_Vendor\Ruckusing_Exception::QUERY_ERROR);
            }
            while ($row = \YoastSEO_Vendor\pg_fetch_assoc($res)) {
                $data[] = $row;
            }
            return $data;
        } else {
            ...
            if (\preg_match($returning_regex, $query, $matches)) {
                if (\count($matches) == 2) {
                    $returning_column_value = \YoastSEO_Vendor\pg_fetch_result($res, 0, $matches[1]);
    ...
    {
        $res = \YoastSEO_Vendor\pg_query($this->conn, $queries);
        if ($this->isError($res)) {
            throw new \YoastSEO_Vendor\Ruckusing_Exception(\sprintf("Error executing 'query' with:\n%s\n\nReason: %s\n\n", $queries, \YoastSEO_Vendor\pg_last_error($this->conn)), \YoastSEO_Vendor\Ruckusing_Exception::QUERY_ERROR);
        }
        return \true;
    }
    /**
    ...
    public function select_one($query)
    {
        ...
        if ($query_type == \YoastSEO_Vendor\SQL_SELECT || $query_type == \YoastSEO_Vendor\SQL_SHOW) {
            ...
            return \YoastSEO_Vendor\pg_fetch_assoc($res);
        } else {
    ...
    {
        return \YoastSEO_Vendor\pg_escape_string($string);
    }
    /**
     * Quote a string
     *
...
SQL;
    ...
    private function db_connect($dsn)
    {
        ...
        if ($db_info) {
            ...
            $this->conn = \YoastSEO_Vendor\pg_connect($conninfo);
            if ($this->conn === \FALSE) {
    ...
    private function load_tables($reload = \true)
    {
        if ($this->_tables_loaded == \false || $reload) {
            ...
            $res = \YoastSEO_Vendor\pg_query($this->conn, $sql);
            while ($row = \YoastSEO_Vendor\pg_fetch_row($res)) {
                $table = $row[0];
    ...
    private function beginTransaction()
    {
        ...
        \YoastSEO_Vendor\pg_query($this->conn, "BEGIN");
        $this->_in_trx = \true;
    }
    /**
    ...
    private function commit()
    {
        ...
        \YoastSEO_Vendor\pg_query($this->conn, "COMMIT");
        $this->_in_trx = \false;
    }
    /**
    ...
    private function rollback()
    {
        ...
        \YoastSEO_Vendor\pg_query($this->conn, "ROLLBACK");
        $this->_in_trx = \false;
    }
}
